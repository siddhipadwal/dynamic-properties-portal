...
const MapPicker = dynamic(() => import('@/components/MapPicker'), { 
  ssr: false,
  loading: () => <div className="h-48 sm:h-64 bg-gray-200 rounded-md flex items-center justify-center">Loading map...</div>
});
      ...
      <header className="bg-white shadow-md">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 py-3 sm:py-4">
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 sm:gap-0">
            <h1 className="text-xl sm:text-2xl font-bold font-lora" style={{ color: '#02333B' }}>Add New Property</h1>
          ...
          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              ...
              <input type="text" name="name" value={formData.name} onChange={handleChange}
                ...
                placeholder="e.g., ALPINE PRIMO" required />
            </div>
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold mb-2" style={{ color: '#02333B' }}>Map Location (Optional)</label>
              <div className="grid grid-cols-2 gap-2 sm:gap-4 mb-2">
                <div>
                  <label className="block text-xs font-bold mb-1" style={{ color: '#02333B' }}>Latitude</label>
                  <input type="text" name="latitude" value={formData.latitude} onChange={handleChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 transition" style={{ '--tw-ring-color': '#B39359' }} placeholder="e.g., 19.1234" />
                </div>
                <div>
                  <label className="block text-xs font-bold mb-1" style={{ color: '#02333B' }}>Longitude</label>
                  <input type="text" name="longitude" value={formData.longitude} onChange={handleChange}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-2 transition" style={{ '--tw-ring-color': '#B39359' }} placeholder="e.g., 72.5678" />
                </div>
              </div>
              <button type="button" onClick={() => setShowMap(!showMap)} className="flex items-center gap-2 px-4 py-2 text-white rounded-md hover:opacity-90 transition" style={{ background: '#B39359' }}>
                {showMap ? 'Hide Map' : 'Pick Location'}
              </button>
              {showMap && <div className="mt-3"><MapPicker onLocationSelect={handleLocationSelect} initialLat={formData.latitude ? parseFloat(formData.latitude) : 19.0760} initialLng={formData.longitude ? parseFloat(formData.longitude) : 72.8777} /></div>}
            </div>
